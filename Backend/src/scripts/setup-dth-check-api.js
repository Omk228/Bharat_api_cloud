...
async function setupDthCheckApi() {
  try {
    console.log('--- Setting up DTH Operator Check in Database ---');

    // 1. Insert/Update upstream_credentials
    const apiKey = process.env.DTH_CHECK_API_KEY || process.env.WAY2API_DTH_API_KEY || process.env.OPERATOR_CHECK_API_KEY || process.env.WAY2API_OPERATOR_API_KEY || process.env.WAY2API_BANK_API_KEY || process.env.WAY2API_EMAIL_API_KEY || 'w2a_b2582c6c952c61b40af38c96917b33a5506ed5cfdf007c6641ea0732ba501bc41c34e1ded9fd917fea13276d24cd8082';
    const baseUrl = process.env.DTH_CHECK_BASE_URL || process.env.WAY2API_DTH_BASE_URL || 'https://app.way2api.com/api/v1/dth/operator-check';
    ...
    await dbPool.query(`
      ...
      ON DUPLICATE KEY UPDATE
        ...
        updated_at = NOW();
    `, [baseUrl, apiKey]);
    console.log('✅ upstream_credentials updated for way2api_dth_master');
    ...
    await dbPool.query(`
      INSERT INTO catalog (
        ...
        'Active',
        'WAY2API Gateway',
        'Instant verification of Indian Direct-to-Home (DTH) subscriber ID / smart card number to identify operator (Tata Play, Sun Direct, Dish TV, Airtel Digital TV, D2H).'
      )
      ON DUPLICATE KEY UPDATE
    ...
    `);
    console.log('✅ catalog updated for api_dth_operator_check');
    ...
    for (const u of users) {
      await dbPool.query(`
        INSERT INTO user_api_pricing (
          ...
          ?, 'api_dth_operator_check', NULL, 1, NOW()
        )
        ON DUPLICATE KEY UPDATE
          is_assigned = 1,
          updated_at = NOW();
    ...
    console.log('--- DTH Operator Check DB Setup Completed Successfully ---');
    await dbPool.end();
  } catch (err) {
    console.error('❌ Error setting up DTH Operator Check API in DB:', err);
    process.exit(1);
  }
}
